...
void Run()
{
	...
	while (!quit)
	{
		...
		while (SDL_PollEvent(&e) != 0)
		{
			...
			{
				SDL_MouseMotionEvent eInvertedY{ e.motion };
				eInvertedY.y = int(g_WindowHeight - e.motion.y);
				OnMouseMotionEvent(eInvertedY);
			}
				//OnMouseMotionEvent(e.motion);
				break;
			case SDL_MOUSEBUTTONDOWN:
			{
				SDL_MouseButtonEvent eInvertedY{ e.button };
				eInvertedY.y = int(g_WindowHeight - e.button.y);
				OnMouseDownEvent(eInvertedY);
			}
